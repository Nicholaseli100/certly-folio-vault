      ...
      <div className="max-w-[1400px] mx-auto px-8 py-5 flex items-center gap-6">
        ...
        <div className="flex-1 flex justify-center">
          <div className="inline-flex items-center gap-1 p-1 rounded-full bg-secondary/80 border border-border/50">
            <button onClick={() => setFilter("all")} className={`${tabBase} ${isActive("all") ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              Todos
            </button>
            <button onClick={() => setFilter("expired")} className={`${tabBase} ${isActive("expired") ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              Vencidos
              <StatusDot kind="expired" count={expiredCount} />
            </button>
            <button onClick={() => setFilter("warning")} className={`${tabBase} ${isActive("warning") ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              A Vencer
...
function StatusDot({ kind, count }: { kind: CertStatus; count: number }) {
  const color = kind === "expired" ? "bg-rose-500 text-white" : kind === "warning" ? "bg-amber-400 text-amber-950" : "bg-emerald-500 text-white";
  return (
    <span className={`${color} text-[10px] font-semibold rounded-full min-w-5 h-5 px-1.5 inline-flex items-center justify-center tabular-nums`}>
      {count}
    </span>
  );
}
